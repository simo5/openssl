                                   ...
                                   const char *event_type)
{
    ...
    *p = OSSL_PARAM_construct_end();
    ...
    status = read_ex_cb(bio, expected, HMAC_LEN, &bytes_read);
    if (status != 1)
        goto err;
    ...
    /* check that the retrieved HMAC is not all zeros and contains an actual
     * HMAC no point in continuing otherwise */
